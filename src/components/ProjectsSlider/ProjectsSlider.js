...
const ProjectsSlider = ({ startFullScreen = false, initialImage = null }) => {
    ...
    useEffect(() => {
        if (initialLoadRef.current && startFullScreen && initialImage) {
            // Find the initial image element in the track
            const initialIndex = images.findIndex(img => img === initialImage);
            const imgElement = imageRefs.current[initialIndex];
    ...
    }, [startFullScreen, initialImage, images]);
    return (
        <LayoutGroup>
            <AnimatePresence>
